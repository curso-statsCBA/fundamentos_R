...
A diferencia de otros paquetes de graficación, incluyendo a las funciones nativas de R que ya hemos visto, la creación de gráficos con *ggplot2* sigue una lógica diferente. Mejor que explicarlo, es mostrarlo!
...
Existen varias formas de exportar los gráficos de *ggplot2*. Una de ellas es a través de la interfaz gráfica de RStudio, en el panel de gráficos. Otra opción es encerrar el bloque de código de la siguiente manera, y correr todo el bloque a la vez:

```{r results=FALSE,eval=FALSE,message=FALSE}
png(filename = "dia4/scatterplot.png", width = 500, height = 400)
ggplot(data = iris, mapping = aes(x = Sepal.Length, y = Petal.Length, color = Species)) +
  geom_point(size = 3, alpha = 0.5) +
  geom_smooth(method = "lm") +
  ggtitle("Scatterplot de ejemplo") +
...
Como hemos visto en el práctico anterior, otras alternativas a `png()` son `jpeg()` y `tiff()`.
...
4. Exporte el panel gráfico con una resolución de 300 <u id='ppi'>ppi</u> dentro de la carpeta "dia4" del proyecto del curso. Para ello, explore `?png`.
...
<p align="center"><a href="pics/meme_practico_ggplot2.png" target="_blank"><img src="pics/meme_practico_ggplot2.png" style="padding: 5px 0 5px 0;width: 50rem;max-width:100%;height:auto;vertical-align:middle;border-style: none;"/></a></p>
