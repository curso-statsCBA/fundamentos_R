...
**Nota:** Antes de continuar es recomendable repasar los [operadores](../dia1/3.1_Operadores.html){target="_blank"} aprendidos en el primer día de clases.
...
```{r}
model_outputs <- data.frame(Species = rep(unique(iris$Species)),
                            Estimate = NA,
                            P.value = NA)
model_outputs
```

Luego, evalúe modelos lineales que relacionen a la variable `Petal.Length` con la variable `Sepal.Length` en ese orden, para cada especie. Rellene la tabla generada anteriormente con los valores de los coeficientes (`Estimate`) y su valor-P (`P.value`) en cada caso. Para hacer esto, utilice alguna estructura de control de flujo aprendida!

*Ayuda*: utilice la función `lm()` para construir los modelos lineales, y la función `summary()` para obtener los coeficientes.
