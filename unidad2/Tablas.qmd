...
Notar la sintaxis para seleccionar una columna, utilizando el símbolo `$`: `tabla$columna`.
...
### Modificación del "tipo" de dato de columnas

Una primera tarea muy usual consiste en cambiar el tipo (o clase) de dato de alguna columna, tras advertir un tipo de dato no adecuado para ciertas columnas (por ejemplo, al correr la función `str()`).

Si asumimos que la columna "Sepal.Length" del dataset `iris` está en formato `character` (lo cual es inadecuado, ya que contiene números decimales), la forma de transformarla es mediante la función `as.numeric()`:

```{r}
iris$Sepal.Length <- as.numeric(iris$Sepal.Length)
```

Exploremos qué estamos haciendo aquí. En primer lugar, selecciono la columna `iris$Sepal.Length` e inmediatamente utilizo `<-` para indicar que dicha columna será reemplazada con los valores que escriba a la derecha. En segundo lugar, los nuevos valores son los valores de la misma columna, pero forzados a tipo numérico: `as.numeric(iris$Sepal.Length)`.

Análogamente, las funciones `as.integer`, `as.character` y `as.logical` transforman un tipo de dato a integer, character y logical.

**La carga de datos a través de tablas con columnas incorrectamente configuradas es uno de los problemas más comunes al comenzar a trabajar con tablas**

### Indexación y filtrado

Otro tipo de tarea muy usual al trabajar con tablas incluye, por ejemplo, obtener una nueva tabla de acuerdo a ciertas condiciones, eliminar ciertas filas o agregar nuevas columnas.
...
Notar que la nueva tabla mantiene la identidad de tabla pero también el orden de las columnas indicadas en la sintaxis. Por lo tanto, esta sintaxis también puede utilizarse para cambiar el orden de las columnas, pudiendo indicar el nombre (como en el ejemplo) o los índices (para este ejemplo sería `c(5, 1)`).

También es posible desprenderse de la coma para seleccionar columnas:
...
**Para probar:** Con la sintaxis anterior, seleccione sólo la columna "Species". Seleccione la columna utilizando tanto la sintaxis que utiliza la coma divisoria como la que desiste de esta. ¿Qué tipo de objeto se devuelve en cada caso?
...
A partir de la tabla `iris`, generar:

-   Una tabla que contenga sólo la especie "setosa", y las columnas "Species" y "Petal.Width".
-   Una tabla que contenga sólo a las especies "setosa" y "versicolor". Deberá utilizar el operador `%in%`, o `|` en en combinación con `==` (recordá de qué estamos hablando en [Vectores](../unidad1/Vectores.qmd)).
-   Una tabla que contenga aquellas observaciones en donde "Petal.Length" es mayor o igual a 4.
-   Una tabla que contenca aquellas observaciones en donde "Sepal.Width" es mayor a 3 o "Sepal.Length" \< 4.

Utilice nombres distintos para cada nueva tabla.
...
- Genere una nueva columna para la tabla `iris` llamada "Flower.color". Para las especies *Iris setosa* E *Iris versicolor*, asignarle "Purple". Para la especie *Iris virginica* asignarle "Blue".
- Utilice la función `unique()` para mostrar combinaciones únicas de especie y color, y la función `table()` para determinar el número de combinaciones para cada caso. *Ayuda: en cada caso al aplicar cada función, debe utilizar 2 vectores.*
...
El uso del punto `.` es útil para hacer el cálculo sobre todas las columnas del dataset en función de la columna definida a la derecha de `~`:
...
Generar una tabla que contenga el cálculo de la media de "Petal.Length" para cada especie. Luego, generar otra tabla con la desviación estándar de "Petal.Length" para cada especie. En cada tabla, cambiar los nombres de las nuevas columnas a "Petal.Length.mean" y "Petal.Length.sd". Finalmente, unir toda la información en una tabla, quedando una columna para especie, otra para la media y otra para la desviación estandar. Ayuda: fusione ambas tablas con `merge()` o `cbind()`.
...
Otras funciones muy útiles asociadas a tablas son `reshape()`, `stack()` y `unstack()`. Estas funciones permiten transformar la **estructura** de nuestros datos, una operación muy común a la hora de preparar datos para el análisis.

Antes de ver las funciones, conviene entender los dos formatos posibles. Imaginemos que tenemos las notas de 3 estudiantes en 2 materias:

**Formato ancho (wide):** cada fila es un sujeto y cada variable medida ocupa una columna distinta.

```
estudiante  matematica  lengua
Juan        8           7
Sofía       9           6
Pedro       7           8
```

**Formato largo (long):** cada fila es una observación, y las variables medidas se apilan en una sola columna. El mismo dato ocupa más filas pero menos columnas.
...
Ningún formato es mejor que el otro; cada uno es más conveniente según el análisis o la visualización que queramos hacer.

La función `reshape()` permite pasar de un formato al otro. Su uso es más complejo que otras funciones de R base, ya que requiere especificar varios argumentos: `idvar` (la variable que identifica a cada sujeto), `varying` (las columnas que van a apilar), `direction` (si queremos ir a `"long"` o a `"wide"`), entre otros.

Las funciones `stack()` y `unstack()` son alternativas más simples para casos concretos:

- `stack()` convierte un data frame en formato ancho a formato largo, apilando varias columnas en una sola. Devuelve un data frame con dos columnas: `values` (los valores) e `ind` (el nombre de la columna de origen).
- `unstack()` hace la operación inversa: a partir de una columna de valores y una columna que indica el grupo, reconstruye el formato ancho.

Son menos flexibles que `reshape()` pero mucho más fáciles de usar cuando la estructura de los datos es simple y no hay variables de identificación complejas. 

**Para probar:** Entra a la ayuda de `reshape()` y `stack()`, y corre los ejemplos para entender mejor cómo funcionan.
...
```{r}
iris_info$year_of_creation <- "Dos mil veintiseis"
...
**Mini ejericio:** Extraiga de `mod_summ$coefficients` (con código, obviamente) el valor de pendiente ("Estimate") para la variable Petal.Length y guardelo en un objeto llamado "est_petal". Ayuda: identifique primero qué tipo de objeto es `mod_summ$coefficients` (también puede asignarlo a un nuevo objeto, si quiere), y a partir de ello, utilice la sintaxis adecuada para la extracción del dato buscado.
...
1.  Cargue la tabla del archivo "analisis_tierra.csv", la cual muestra los resultados de un análisis de muestras de tierra, detallando las concentraciones de N, C y Ca. 
2.  Explore su estructura con `str()` y las primeras filas con `head()`. ¿Nota algún error? Ayuda: para cargar la tabla, deberá utilizar `read.table()` con los argumentos `sep` (que indica qué caracter separa los datos entre columnas) y `dec` (que indica qué caracter se utiliza como símbolo decimal).
3.  Explore nuevamente su estructura y contenido utilizando las funciones `head()`, `tail()`, `str()` y `dim()`. 
4.  Explore los nombres de las columnas. Corrija el error en la segunda columna, que debería llamarse "Parcela".
5.  Hay valores negativos para la concentración de Ca, lo cual no tiene sentido. Cambie estos valores por ceros.
6.  Hay valores `NA` en la columna que indica la concentración de N. Genere una nueva tabla, utilizando el mismo nombre que antes, que no contenga estas observaciones (filas). Puede utilizar la expresión `is.na()`, pero explore también la ayuda de `na.omit()`. Ayuda: `is.na(objeto)` pregunta si el objeto es igual a `NA`, mientras que `!is.na(objeti)`pregunta si NO es igual a `NA`.
7.  Cargue la tabla del archivo "analisis_tierra2.csv", la cual muestra los resultados realizados por otra técnica (Claudia). Incorpore estos nuevos datos a la tabla anterior. Es necesario corregir algunos errores antes de hacer esto <img src="pics/emoticon1.png" width="20" height="20"/>. Entre ellos, los números asignados a cada muestra deben cambiarse, para que estos no coincidan con los números de muestra de la tabla anterior.
8.  Actualice la tabla ordenándola según la Parcela (orden alfabético) y por valores crecientes en la cantidad de N, en ese orden!
9.  Transforme a la columna "Tecnico" a factor.
10.  Genere una tabla de contingencia considerando la variable "Tecnico" y "Parcela".
11.  Genere una tabla resumen con los valores medios de N, C y Ca por Parcela, y otra con los valores de desviación estándar de N, C y Ca por Técnico. Exporte dichas tablas resumen.
