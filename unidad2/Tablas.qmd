...
4.  Hay valores `NA` en la columna que indica la concentración de N. Genere una nueva tabla, utilizando el mismo nombre que antes, que no contenga estas observaciones (filas). Puede utilizar la expresión `is.na()`, pero explore también la ayuda de `na.omit()`. Ayuda: `is.na(objeto)` pregunta si el objeto es igual a `NA`, mientras que `!is.na(objeti)`pregunta si NO es igual a `NA`.
...
6.  Actualice la tabla ordenándola según la Parcela (orden alfabético) y por valores crecientes en la cantidad de N, en ese orden!
7.  Transforme a la columna "Tecnico" a factor.
