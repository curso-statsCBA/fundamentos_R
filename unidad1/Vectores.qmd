...
Las funciones `is.numeric()`, `is.integer()`, `is.logical()`, `is.character()` e `is.null()` "preguntan" si la clase del objeto entre paréntesis es numeric, integer, logical, character o NULL, respectivamente.
...
La función `seq_along()` admite un único argumento (`along.with`), para el que espera un objeto (típicamente un vector, pero admite otros tipos). Esta función genera una secuencia de números enteros, desde el 1 hasta la longitud del objeto provisto. Por ejemplo:
...
1.  Utilice las funciones `rep()`, `seq()` y `seq_along()` para generar los siguientes vectores:

-   Una sucesión de números pares de 15 elementos.
-   Una sucesión de números decimales, con un incremento de 0.15.
-   Una sucesión de números enteros que comience con el 1 y continúe hasta el número igual la longitud del vector `c(25, 78, 41, 79, 11, 6, 8)`.
