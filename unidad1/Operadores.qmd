...
Es el operador que utilizamos para crear objetos en R. En R, utilizamos la combinación `<-`. Por ejemplo, el siguiente código crea un objeto llamado `a` al que le asignamos en número 5:

```{r}
a <- 5
```

Aunque no es común, el orden de la asignación puede escribirse al revés, invirtiendo el sentido de asignación (`->`):

```{r}
5 -> a
```

Esta combinación de símbolos puede interpretarse como una "flecha" que indica el valor que asignamos (el número 5) a un objeto al que le ponemos nombre (`a`).

Si además de crear el objeto, lo escribimos entre paréntesis, el objeto es impreso en la consola:

```{r}
(a <- 5)
```

Es posible asignar el mismo valor a varios objetos simultáneamente:

```{r}
a <- b <- c <- 5
```

En R, también podemos utilizar el símbolo `=` para las asignaciones:

```{r}
a = 5
```

Sin embargo, para asignaciones siempre recomendamos utilizar el símbolo tradicional `<-`, y así evitar confusiones con el símbolo `=` que se utiliza al definir argumentos o parámetros de funciones, o con el símbolo `==`, que es un [operador relacional](#op_relacionales).
...
### Operadores relacionales {#op_relacionales}
...
Aquí vemos que el objeto `d` no existe en el ambiente de R. Sin embargo, el resultado es `FALSE` porque la primera condición ya es falsa. Si hiciéramos la misma comparación con `&`, R devuelve un error:
...
Si todas las condiciones son falses, también obtenemos `FALSE`:
