...
```{r }
asistentes <- c("Darío", "Ana Paula", "Carina", "William", "Trinidad",
                "Axel", "Martina", "Luciana", "Nicolás", "Ana", 
                "Sofía", "Lucrecia", "Clara")
...
#### *La función `seq()` y `seq_along()` en un bucle `for`*
...
La función `seq_along()` también es útil en este context, cuando se desea iterar sobre los elementos de un vector ya creado previamente:

```{r}
especies <- c("Algarrobo", "Quebracho", "Espinillo") 
# Secuencia de 1 a 3, utilizando un vector generado previamente
...
[Descargue el archivo zip](https://github.com/pastornicolas/fundamentos_R/raw/6263d3cc456bb84cce07ce2d28bef5068cf2b6dd/unidad3/data/iris_more.zip) y extraiga el archivo denominado "iris_more.csv" en el directorio "día 4". Cargue la tabla:

```{r}
iris_more <- read.table("data/iris_more.csv")
```

Esta tabla es idéntica al dataset iris con el que hemos estado trabajando, pero contiene los datos de un total de 15 especies. En primer lugar, construya el siguiente `data.frame` vacío:

```{r}
model_outputs <- data.frame(Species = unique(iris_more$Species),
                            Estimate = NA,
                            P.value = NA)
model_outputs
```

Luego, evalúe modelos lineales que relacionen a la variable `Petal.Length` con la variable `Sepal.Length` en ese orden, para cada especie. Rellene la tabla generada anteriormente con los valores de los coeficientes (llamado `Estimate` o coeficiente de la variable predictora) y su valor-P (`P.value`) en cada caso. La idea no es hacer esto manualmente, sino utilizar alguna estructura de control de flujo aprendida!
...
- Intente rellenar los datos de una sola especie. Luego intente replicarlo al resto mediante una estructura de control de flujo.
- Utilice "pseudocógigo" para descifrar los pasos a seguir: con palabras, describa cada pequeño paso de la estructura a generar. Luego, traduzca el pseudocódigo a código R.
...
Se dispone de un archivo de texto llamado `secuencias.txt` que contiene una secuencia de ADN por línea:
...
```r
con <- file("secuencias.txt", "r")
linea <- readLines(con, n = 1)
n_linea <- 1

while(length(linea) > 0) {
  if(grepl("GTAC", linea)) {
    cat("GTAC encontrada en:\n")
    cat(linea, "\n")
    cat("Número de línea:", n_linea)
    break
  }
  linea <- readLines(con, n = 1)
  n_linea <- n_linea + 1
...
En el código se muestran algunas funciones que no hemos visto:

- `file()`

Abre una conexión con el archivo. El argumento `"r"` indica que el archivo se abrirá en modo lectura (*read*). El objeto `con` almacena la conexión que permite acceder al contenido del archivo.

- `readLines()`

Lee líneas desde la conexión. El argumento `n = 1` indica que se debe leer una sola línea cada vez. Cada nueva llamada a `readLines()` continúa leyendo desde donde quedó la lectura anterior.

- `grepl()`

Busca un patrón de texto dentro de una cadena de caracteres. Devuelve `TRUE` si encuentra el patrón y `FALSE` en caso contrario.

- `close()`

Cierra la conexión con el archivo una vez finalizada la lectura. Es una buena práctica cerrar las conexiones abiertas para liberar recursos del sistema.

**Consignas:**

- Inteprete el código, línea por línea.
- ¿Qué resultado obtendré en la consola al correr el bloque entero de código?
- En este caso, tiene más sentido usar `while` que `for`. ¿Por qué? Imagine que el archivo `secuencias.txt` tuviese cientos de miles de líneas.
