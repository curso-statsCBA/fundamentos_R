...
[Descargue el archivo](https://raw.githubusercontent.com/pastornicolas/fundamentos_R/bc66515696d691eba6ced8feb4ee12b66b2b7d56/unidad3/data/iris_more.csv) denominado "iris_more.csv", y cargue la tabla:

```{r}
iris_more <- read.table("data/iris_more.csv")
```

Estaa tabla es idéntica al dataset iris con el que hemos estado trabajando, pero contiene los datos de un total de 15 especies. En primer lugar, construya el siguiente `data.frame` vacío:
...
for(i in 1:nrow(model_outputs)){
  mod <- lm(Petal.Length ~ Sepal.Length, data = iris_more[iris_more$Species == model_outputs$Species[i], ])
  summ <- summary(mod)
  model_outputs$Est[i] <- summ$coefficients[2, 1]
  model_outputs$P.value[i] <- summ$coefficients[2, 4]
}
# Respuesta verNico
for (sp in unique(iris_more$Species)){ 
  mod <- with(iris_more[iris_more$Species == sp,], lm(Petal.Length ~ Sepal.Length))
