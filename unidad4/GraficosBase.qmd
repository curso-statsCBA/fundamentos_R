...
A pesar del alto nivel de personalización que se puede lograr con el apartado gráfico de `R base`, generalmente lo usaremos para realizar gráficos exploratorios o como parte de las funciones gráficas de otros paquetes, reservando a paquetes especializados en representación gráfica para elaborar figuras *finales*.

```{r, echo=FALSE}
tippy::tippy_this(elementId = 'comentario', interactive= 'true', tooltip = "<span style='font-size:14px;'>Existen otros paquetes que profundizan funcionalidades específicas que trascienden los objetivos de este curso. Aquí puede encontrar <a href='https://www.geeksforgeeks.org/top-r-libraries-for-data-visualization/' target='_blank'>una lista de los paquetes top en 2025</a> según un sitio especializado.</span>")
```

Para entender el apartado gráfico nativo, definiremos un concepto relativo al *nivel* de la función utilizada. Cuando realicemos gráficos podremos utilizars funciones de *alto nivel*, que son aquellas funciones que crean nuevos gráficos, generalmente junto a los principales ejes, etiquetas ó títulos (e.g. `plot()`, `hist()`, etc.); y funciones de *bajo nivel*, que son aquellas con las que podremos añadir elementos extras a un gráfico ya creado, como puntos, líneas, flechas u otras etiquetas (e.g. `points()`, `text()`, etc.). En `R base`, la **combinación de funciones de alto y bajo nivel es la que permite realizar una enorme personalización de los gráficos**.

En cuanto a los **tipos de gráficos**, las posibilidades que ofrece R son realmente ilimitadas y podrían constituir en sí mismo un curso de representación gráfica. Pero de forma general podemos mencionar a:
...
### Explorando graficamente los datos

Cuando trabajamos con datos, es deseable que nos familiaricemos con las variables que estamos utilizando en nuestros análisis. Explorar gráficamente las características de nuestras variables puede proveernos de mucha información, incluso antes de realizar análisis estadísticos puntuales. Realizar estas exploraciones en `R`resulta extremadamente sencillo, para ver gráficamente una variable, bastará con que usemos la función `plot()`.
...
Como podemos ver, lo que obtuvimos fue un gráfico de dispersión representando todos los valores numéricos de la variable "Ancho de sépalo" para el conjunto de datos *Iris*. Siguiendo con este ejemplo, si quisiéramos ver la forma de la distribución de nuestros datos, podríamos utilizar la función `hist()` para realizar un histograma.
...
O también, podríamos estar interesados en estudiar la relación de 2 variables que tengamos en nuestro conjunto de datos.
...
Notemos como en la expresión precedente, utilizamos lo que en R se conoce como *sintaxis de fórmula*, en donde el operador `~` significa "*en función de*". En este caso sería "largo de sépalo *en función de* largo de pétalo", es decir, el ancho en el eje de las ordenas y el largo en el eje de las abscisas.

Esta forma de solicitar gráficos a R tiene la ventaja de detectar automáticamente el tipo de dato, y reaccionar dela forma indicada según los mismo. Por ejemplo, veamos que pasa si queremos ver como se relaciona una variable numérica con una categórica.
...
### La función `plot()` sobre salidas de otros paquetes

La gran mayoría de paquetes de análisis disponibles en R poseen alguna salida gráfica asociada. Muchas veces, estos gráficos no son potencialmente usables como versiones finales, pero permiten evaluar cuestiones del análisis vinculadas al funcionamiento del paquete específico. Cuando la salida de la ejecución de un paquete tiene información para realizar representaciones gráficas, se puede acceder a esta con la misma función `plot()` del apartado gráfico básico.

Para ejemplificar esto, pensemos que queremos realizar una regresión lineal y evaluar si los supuestos de la misma se cumplen. Para ellos, hagamos primero un modelo de regresión lineal entre las variables `Petal.Length` y `Sepal.Length`, para la especie *I. virginica* del conjunto de datos `Iris`:

```{r eval=T, echo=T}
# Creando y viendo el resultado del modelo de regresión lineal
regL <- lm(Petal.Length ~ Sepal.Length, data = subset(iris, Species == "virginica"))
summary(regL)
```

Como podemos ver, el objeto `regL` es dónde guardamos la salida de la función `lm()` que realiza el ajuste del modelo lineal. Para acceder a los resultados de este análisis usamos la función `summary()`, pero ¿qué sucedería si pasamos este mismo objeto a la función `plot()`?

```{r eval=F, echo=T, fig.align='center', fig.height=4, fig.width=4}
# Gráficos de diagnóstico del modelo de regresión lineal
plot(regL)
```
```{r eval=T, echo=F, fig.align='center', fig.height=8, fig.width=8}
par(mfrow=c(2,2), mar=c(5,5,1.5,1))
plot(regL)
```

En este caso, **la función gráfica invoca a un método específico relativo al tipo (*clase*) de objeto sobre el que está operando**. Esto produce una serie de gráficos que en este caso sirven para evaluar los supuestos matemáticos de la regresión lineal. Esto es, además, un ejemplo de la característica de *polimorfismo* que vimos en la [clase introductoria](../unidad1/IntroR.qmd){target="_blank"}.
...
Siguiendo con nuestro ejemplo, para mostrar los 4 gráficos juntos configuraremos con `par()` la posibilidad de tener un panel de 4 gráficos, con 2 filas y 2 columnas, esto es: `mfrow = c(2,2)`.

```{r eval=F, echo=T, fig.align='center', fig.height=8, fig.width=8}
# Gráficos de diagnóstico del modelo de regresión lineal
par(mfrow = c(2,2))
plot(regL)
```
...
La sintaxis a recordar es que el argumento del parámetro `mfrow` es siempre un vector de dos elementos, que se corresponden con el número de filas y columnas deseadas. Así podemos, por ejemplo, solicitar que los 4 gráficos se dispongan en una misma fila y cuatro columnas:

```{r eval=F, echo=T, fig.align='center', fig.height=4, fig.width=10}
# Gráficos de diagnóstico del modelo de regresión lineal
par(mfrow = c(1,4))
plot(regL)
```
```{r eval=T, echo=F, fig.align='center', fig.height=4, fig.width=10}
# Gráficos de diagnóstico del modelo de regresión lineal
par(mfrow = c(1,4), mar=c(5,5,1.5,1))
plot(regL)
```


## Personalización de gráficos (sección opcional para consulta futura)

Las posiblidades de personalización utilizando el sistema gráfico de `R base` son muy amplias. Intentaremos a continuación abordarlo desde una perspectiva progresiva, partiendo de los gráficos simples elaborados con funciones de alto nivel, para luego ir sumando parámetros y funciones de bajo nivel, haciendo más complejo nuestro código para definir los principales elementos que componen un gráfico.
