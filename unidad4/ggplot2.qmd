...
## Gráficos en paneles o facetas

Las facetas permiten dividir un gráfico en múltiples paneles según los niveles de una variable categórica, facilitando la comparación entre grupos. `ggplot2` ofrece dos funciones para esto: `facet_wrap()` y `facet_grid()`.

`facet_wrap()` toma una sola variable y distribuye los paneles en una grilla que se acomoda automáticamente. Con el argumento `ncol` o `nrow` podemos controlar cuántas columnas o filas tendrá la grilla.

En el siguiente ejemplo graficamos la relación entre el largo y el ancho del sépalo, con un panel para cada especie:
...
Los tres paneles comparten los mismos ejes, lo que facilita comparar la dispersión y los rangos entre especies.

`facet_grid()` cruza **dos variables**: una define las filas y otra las columnas de la grilla. Esto permite ver simultáneamente cómo varía una relación en función de dos factores a la vez.

Como `iris` solo tiene una variable categórica natural (`Species`), creamos una segunda clasificando el tamaño del pétalo:

```{r}
iris$petal_size <- ifelse(iris$Petal.Length <= 4, "petalo chico", "petalo grande")
```

Luego usamos `facet_grid()` con `petal_size` en las filas y `Species` en las columnas:

```{r}
ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width)) +
  geom_point() +
  facet_grid(petal_size ~ Species) +
  theme(aspect.ratio = 1, )
```

La grilla resultante tiene 2 filas × 3 columnas = 6 paneles. Puede observarse que *setosa* directamente no tiene "pétalos grandes", por lo que ese panel queda vacío. Esto en sí mismo es un resultado informativo.
